...
- Расписание собирается ЧАСАМИ, а не одной лентой. Час = сидячий блок, один
  подъём, крупная сцена, одна посадка, сидячий блок. Раньше бит выходил ровно
  каждые пять минут в одном и том же порядке, а стоячее движение попадало между
  двумя сидячими — он вставал ради одного жеста и садился обратно.
- Каждое движение получило слот: 72 слота на 43 действия у Данте, 64 на 31 у
  Корвина. Раздача круговая — любое выходит раньше, чем какое-то повторится.
- Ровно одна крупная сцена в час, и каждый час другая: круг не повторяется
  девять часов у Данте и восемь у Корвина.
- Порядок микро-жестов внутри блока позы тасуется при каждом запуске.
- Крупные сцены больше не глушит ничто — ни печать, ни игра, ни музыка.
- Веса холостой лотереи выровнены: было от 19.8% до 0.8%, стало 8.2% у каждого.
  Пять движений забирали 69% выборов, и хвост вроде `nap` или `cleansword` не
  показывался почти никогда. Уместность теперь держит расписание.
- Под музыку молчат 42 сцены Данте из 43 — он слушает вместе с тобой. Мото
  исключение: ехать под трек это и есть быть в теме. Все три мото-сцены теперь
  и в кино, и под музыку; Корвину дан свой музыкальный круг.
- Барбекю вынесено из просмотра видео в игру: 21-я минута, раз в 45 минут.
- Перезапуск ECHO при работающей игре начинает сессию с нуля.

### Исправлено

- Поза перестала скакать. Решений о позе было два и они расходились: мост
  смотрел на разрешённую позу, а `posture(state)` — на имя состояния, и для
  `coding` это «стоя». Мост решал «сидим», следующая строка молча поднимала
  флаг. Замер: 36 смен позы за прогон → 2 на 44 события за четыре минуты.
- Четыре пары сигналов с противоположными позами: `thinking`↔`coding`,
  `idle`↔работа, печать↔работа, плановый бит↔рабочая поза.
- Ограничено ожидание позы Корвина: незакрытый флаг подвешивал сцену навсегда,
  и вместе с ней весь companion.
- Лаунчер Steam больше не держит игровую сессию открытой бесконечно.
- Медиа-детект больше не ждёт подтверждения от системной media-сессии: решает
  видимый заголовок окна, реакция меньше трёх секунд вместо десяти.
- `takePlanned` не смотрит на вес и не бросает кости.
- Кадры сцен удаления попали в репозиторий — код на них ссылался, а PNG не были
  закоммичены, и в собранном артефакте эти сцены рисовались битыми.
